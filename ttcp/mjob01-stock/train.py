...
try:
    ...
    print("[MLflow] clush_mlflow_logger not found — MLflow logging disabled")
...
# ============================================
def setup():
    # GPU 가용 여부에 따라 backend 자동 선택 (env로 override 가능)
    try:
        gpu_available = torch.cuda.is_available() and torch.cuda.device_count() > 0
    except Exception:
        gpu_available = False

    default_backend = "nccl" if gpu_available else "gloo"
    backend = os.environ.get("DIST_BACKEND", default_backend)
    print(f"GPU available: {gpu_available}, using backend: {backend}")
...
def get_gpu_info():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            for i, line in enumerate(lines):
                ...
                # [N/A] 포함 시 PyTorch fallback 사용
                if '[N/A]' in mem_used or '[N/A]' in mem_total:
                    device = torch.cuda.current_device()
                    mu = torch.cuda.memory_allocated(device) / 1024**2
                    mt = torch.cuda.get_device_properties(device).total_memory / 1024**2
                    gpu_info.append(f"GPU{i}: {util}% | {mu:.0f}/{mt:.0f}MB")
                else:
    ...
    return f"GPU{device}: {mem_used:.0f}/{mem_total:.0f}MB"
...
class StockNewsTransformer(nn.Module):
    ...
    def forward(self, price_seq, news_embed):
        ...
        # news_emb 이 all-zero 이면 key/value 가 zero → attn output 도 zero.
        # zero price_encoded + zero combined = zero → LayerNorm 에서 NaN 가능.
        # key/value 에 eps-noise 를 더해 분산 0 을 방지한다.
        news_emb_safe = news_emb + 1e-6
...
def prepare_data(data_dir, seq_length=30, pred_length=5, rank=0, chromadb_dir="/mnt/storage/chromadb", skip_news_fetch=False):
    ...
    # NaN 가격 행 제거 (yfinance 가 휴일·결측 행에 NaN 반환하는 경우 대비)
    nan_mask = np.isnan(all_prices).any(axis=1)
    if nan_mask.any():
        n_nan = int(nan_mask.sum())
        print(f"[Rank {rank}] Removing {n_nan} NaN price entries: "
              f"{[all_dates[i] for i in np.where(nan_mask)[0]]}", flush=True)
        all_prices = all_prices[~nan_mask]
        all_dates  = [d for d, m in zip(all_dates, nan_mask.tolist()) if not m]
    ...
    print(f"[Rank {rank}] Total available samples: {len(train_data['X_price'])}")
    ...
    with torch.no_grad():
        for price, news, target in test_loader:
            ...
            if torch.isnan(output).any():
                print(f"[Eval] NaN detected in model output — "
                      f"price: min={price.min():.4f} max={price.max():.4f}, "
                      f"news all-zero: {(news.abs().sum() == 0).item()}",
                      flush=True)
                return float('nan')
...
def main():
    ...
    parser.add_argument('--num-train-samples', type=int, default=-1, help='최대 학습 샘플 수 (-1: 전체 사용)')
    parser.add_argument('--sweep-csv', type=str, default='', help='Sweep 결과 CSV 경로 (clush_mlflow_logger.log_sweep_csv 로 기록)')
    parser.add_argument('--mlflow-tracking-uri', type=str, default='', help='MLflow tracking server URI (overrides MLFLOW_TRACKING_URI env var)')
    ...
    gpu_available = torch.cuda.is_available() and torch.cuda.device_count() > 0
    device = torch.device(f'cuda:{local_rank}') if gpu_available else torch.device('cpu')

    if rank == 0:
        _gpu_name = torch.cuda.get_device_name(device) if gpu_available else 'CPU'
        print(f"[Sweep] gpu_model={_gpu_name}")
    ...
    # ── MLflow 로거 초기화 (rank 0 only) ────────────────────────────
    # NODE_RANK: torchrun --node_rank 에서 설정. DDP 미연결 시 각 노드가
    # 독립 그룹(rank=0)이 될 수 있으므로 NODE_RANK env var 로 이중 확인.
    mlflow_logger = None
    _node_rank = int(os.environ.get("NODE_RANK", "0"))
    if rank == 0 and _node_rank == 0 and _MLFLOW_AVAILABLE:
        _run_name = (
            f"n{world_size}_e{args.epochs}_b{args.batch_size}"
            f"_d{args.d_model}_l{args.num_layers}"
        )
        mlflow_logger = ClushMLflowLogger(
            experiment_name="stock-sweep",
            run_name=_run_name,
    ...
    if mlflow_logger is not None:
        mlflow_logger.log_dataset_info(train_dataset, test_dataset)
    # ─────────────────────────────────────────────────────────────────
    ...
    # ── Meta: 모델 정보 수집 (torchinfo) ──────────────────────────
    total_params = 0
    model_size_mb = 0.0
    if rank == 0:
        total_params = sum(p.numel() for p in model.parameters())
        model_size_mb = round(sum(p.numel() * p.element_size() for p in model.parameters()) / 1024 ** 2, 2)
        if torchinfo_summary is not None:
            try:
                dummy_price = torch.zeros(1, args.seq_length, 1).to(device)
                dummy_news  = torch.zeros(1, args.seq_length, 768).to(device)
                info = torchinfo_summary(model, input_data=(dummy_price, dummy_news), verbose=0)
                total_params = info.total_params
                model_size_mb = round(info.total_param_bytes / 1024 ** 2, 2)
            except Exception as e:
                print(f"torchinfo warning: {e}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if rank == 0:
            ...
            if mlflow_logger is not None:
                mlflow_logger.log_epoch_end(epoch, avg_loss, test_loss,
            ...
            is_best = (not (test_loss != test_loss)) and test_loss < best_loss  # NaN-safe
            if is_best:
                best_loss = test_loss
                no_improv_count = 0
                writer.add_scalar('Loss/best', best_loss, epoch)
            elif not (test_loss != test_loss):  # NaN이 아닌 경우만 카운트
                no_improv_count += 1
    ...
    if rank == 0:
        ...
        # ── Sweep CSV 기록 (clush_mlflow_logger.log_sweep_csv) ────────
        training_time_sec = round(time.time() - train_start_time, 1)
        print(f"[Sweep] training_time_sec={training_time_sec}  best_loss={round(best_loss, 6)}")

        if args.sweep_csv and mlflow_logger is not None:
